...
class AuthController extends Controller
{
  ...
  $request['password']=decode5t($request->password); #SKP
      ...
      if(count($data)>0 && $data[0]->role == $request->role)
       {
            ...
            }else
            {
                ...
                return back()->with('fail','Your Account is not Actived Yet. please Contact website Adminstrator to Active your Account');
       ...
       if(count($data)>0 && $data[0]->role == $request->role)
        {
...

            {
                ...
                return redirect()->intended('/landing')->with('falils','Your registration is successfull. your account is not actived yet. please contact your adminstrator to active your account');
   ...
   $userEmailDomain = emaildomeins::where('emaildomain',$new_array[1])->get();
    ...
    else if ( isset($request->email) && trim($request->email) =="" ) {
        return response()->json([
            'error'   => 1,
